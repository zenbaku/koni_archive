...
/// metadata, never materialized or followed (§7).
...
/// (§4). Whether content is compressed at all is derivable: everything
/// except [stored] is.
...
/// One entry of an archive — deeply immutable and isolate-transferable (§6).
///
/// [path] is always normalized at parse time (§7): `/` separators, no drive
...
final class ArchiveEntry {
  ...
  ArchiveEntry({
    ...
    this.pathEscapedRoot = false,
         ...
         'modified must be UTC (§4); readers normalize at parse time',
       );
  ...
  /// entries. Metadata only — never followed (§7).
  ...
  /// archive root (via `..`) and was sanitized (§7).

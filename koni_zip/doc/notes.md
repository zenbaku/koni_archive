...
Decisions made where the PKWARE APPNOTE is ambiguous, matched against
the reference tools (Info-ZIP zip/unzip).
...
time is exposed *as if* UTC (documented lossiness). When a `UT`
...
yields the target (never followed).
...
`SizeLimitExceededException`, the bomb guard, detected incrementally;
...
- Multi-volume EOCD (disk numbers, incl. nonzero ZIP64 disk fields) →
  `UnsupportedFeatureException` (non-goal).
...
from wrapping negative on the VM (found by the fuzzer).
...
escape hatch. No committed mojibake fixture exists because no

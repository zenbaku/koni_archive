...
| Feature | Notes |
| --- | --- |
| Single-member `.gz` | opens as a single-entry archive |
...
| FEXTRA / FCOMMENT / FHCRC | parsed (FHCRC verified); 1 MiB header cap |
...
| Feature | Status |
| --- | --- |
| Random access within `.gz` (zran-style seek index) | deferred |
| Sequential (non-seekable) input | out of scope Phase 1 |

## Spec references

- RFC 1952 (gzip file format), RFC 1951 (DEFLATE, via koni_codecs)
- gzip(1) observed behavior as the reference tool
